<template>
    <div class="setting-doctor new-addition blog">
        <breadcrumb name="Tənzİmləmələr" icon="fa-cog" :routes="[{url:'/profile/cpp#/doctor/' + id,name:'Həkimlər'},{name:'Tənzimləmələr'}]"></breadcrumb>
        <div class="row">
            <div class="col-md-7">
    ...
    export default {
        ...
        methods: {
            submitFile() {
                ...
                axios.post('/doctor/edit', formdata).then (
                    res =>{
            ...
            getDegree: function () {
                axios.get('/doctor/list-academic-degree',this.Doctor_data).then(
                    response=> {
            ...
            getSpecialist: function () {
                axios.get('/doctor/specialists',this.Doctor_data).then(
                    response=> {
                        if (response.data.status !== 200) {
                            return Swal.fire({
                                type: 'error',
            ...
            getDoctorData:function () {
                axios.get('/doctor/info/'+this.id,this.Doctor_data).then(
                    response=> {
            ...
            getSocials: function () {
                axios.get('/doctor/list-sosial-link').then(
                    response=> {
                        if (response.data.status !== 200) {
                            return Swal.fire({
                                type: 'error',
