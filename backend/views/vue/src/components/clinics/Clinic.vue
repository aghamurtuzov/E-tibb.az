<template>
    <div class="clinic-inner blog">
        ...
        <div class="row">
            <div class="col-md-12 clinic-position"  v-if="ClinicData.enterprise">
                <div class="row relative">
                    <div class="col-lg-6 col-md-12">
                        <div class="clinic-information">
                            <div class="block">
                                <div class="logo-login">
                                    <img v-if="ClinicData.enterprise.thumb" :src="ClinicData.enterprise.thumb" alt="klinika" class="img-responsive">
                                    <img v-else src="/admin/cp/img/object-new.png" alt="klinika" class="img-responsive">
                                    <p>Qeydiyyat nömrəsi
                                        <span>{{ClinicData.enterprise.user_id}}</span>
                                    </p>
                                </div>
                                <div class="clinic-basics">
                                    <p>Adı</p>
                                    <h3>{{ClinicData.enterprise.name}}</h3>
                                </div>
                                <address>
                                    <ul class="list-unstyled">
                                        <li>
                                            <p>E-mail</p>
                                            <span>{{ClinicData.enterprise.email}}</span>
                                        </li>
                                ...
                                <div class="clinic-edit" >
                                    <button class="btn btn-blocked" @click="blockEnterprise()" v-if="ClinicData.enterprise.status == 1">
                                        BLOCK ET
                                    </button>
                                    <button class="btn btn-confirm" @click="activeEnterprise()" v-else>
                                        Təsdİq et
                                    </button>
                                    <router-link :to="{path:'/enterprises/setting/' + id+'?id=1'}" class="btn btn-edit">
                                        DÜZƏLİŞ et
                                    </router-link>
    ...
    export default {
        ...
        methods:{
            getClinicData:function () {
                axios.get('/enterprises/info/'+this.id).then(
                    response=> {
                        ...
                        if(!this.phone_check) {
                            for(let i of this.ClinicData.phones){
                                this.phones["a"+i.number_type].push(i.number)
                            }
                            this.phone_check=true;
                        }
                        let clinic_address = [];
                        for (let i in this.ClinicData.addresses){
                            clinic_address.push(this.ClinicData.addresses[i].name)
                        }
                        this.clinic_address = clinic_address.join(', ');
                    })
            },
            getClinicPayments:function (page=1) {
            ...
            blockEnterprise:function () {
                if(this.ClinicData.enterprise.status =="0") return;
                axios.post('/enterprises/block ',{id: this.ClinicData.enterprise.id}).then(
                    response=> {
                        this.ClinicData.enterprise.status = (this.ClinicData.enterprise.status == "1") ? 1 : 0;
                        this.getClinicData()
                    });
            },
            activeEnterprise:function () {
                if(this.ClinicData.enterprise.status =="1") return;
                axios.post('/enterprises/active',{id:this.ClinicData.enterprise.id}).then(
                    response=> {
                        this.ClinicData.enterprise.status = (this.ClinicData.enterprise.status == "0") ? 0 : 1;
                        this.getClinicData()
                    }
                )
            },
            sendMessages(url) {
                let id = this.ClinicData.enterprise.id;
