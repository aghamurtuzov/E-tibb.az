...
class ConsultationApiModel extends Model
{
    ...
    {
        return Yii::$app->db->createCommand("SELECT c.*,d.`name` as `doctor_name` FROM `site_consultation` AS c INNER JOIN `site_doctors` AS d ON d.`id`=c.`doctor_id` WHERE c.`doctor_id`=:doctor_id AND c.`status`<>:status ORDER BY c.id DESC LIMIT {$limits[0]},{$limits[1]}",[':doctor_id'=>$doctor_id,':status'=>self::STATUS_DELETED])->queryAll();
    }
