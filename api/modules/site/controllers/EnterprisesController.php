    ...
    public function actionClearPhoneNumber()
    {
        ...
        if(!empty($numbers))
        {
            foreach($numbers as $key => $number)
            {
                ...
                $n = str_replace(' ','',$n);
                $n = str_replace('(+99455)','55',$n);
                $n = str_replace('(+99451)','51',$n);
                $n = str_replace('(+99450)','50',$n);
                $n = str_replace('(+99470)','70',$n);
                $n = str_replace('(+99477)','77',$n);
                $n = str_replace('+994(51)','51',$n);
                $n = str_replace('(+994570)','70',$n);
                $n = str_replace('(055)','55',$n);
                $n = str_replace('(050)','50',$n);
                $n = str_replace('(070)','70',$n);
...
//        }
        ...
        $array = [];
        $enterprises = SiteEnterprises::find()->where(['update'=>0,'user_id'=>0])->all();
        ...
        if(!empty($enterprises))
        {
            foreach($enterprises as $key => $enterprise)
            {
                ...
                if(empty($check))
                {
                    ...
                    $user->email        = $email;
                    $user->phone_number = !empty($number) ? $number['number'] : '0550000000';
                    $user->status       = 1;
                    ...
                    if($user->save())
                    {
                        if(!empty($user->phone_number))
                        {
                            ...
                            $array['ok'][$key]['enterprise_id'] = $enterprise['id'];
                            ...
                            $login    = 'ona.az';
                            $msisdn   = $mobile;
                            $text     = "E-mail:{$email}/Shifre:{$password}";
                            $sender   = 'E-tibb.az';
                            $password = 'ona2019az';
                            ...
                            //$ok = $functions->SendRequestWithCurl($sendSms);
                        }
                        ...
                        $array['no'][$key]['error'] = var_dump($user->errors);
                        $array['no'][$key]['enterprise_id'] = $enterprise['id'];
                    }
                }else{
                    $array['indb'][$key]['enterprise_id'] = $enterprise['id'];
                }
            }
        }
        return $array;
    ...
    public function actionIndex()
    {
        ...
        $totalCount = $model->EnterpriseCount($cat_id,$search);
        ...
        $list = $model->Enterprises($cat_id,$limits,$search);
