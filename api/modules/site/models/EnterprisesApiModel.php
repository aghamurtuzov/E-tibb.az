...
class EnterprisesApiModel extends Model
{
    ...
    /** Obyektler */
    public function EnterpriseCount($cat_id,$search)
    {
        if(strlen($search)>=3)
        {
            return Yii::$app->db->createCommand("SELECT count(e.`id`) AS `count` FROM `site_enterprises` AS e INNER JOIN `site_users` as u ON e.`user_id`=u.`id` WHERE e.`category_id`=:cat_id AND (e.`name` LIKE '%$search%' OR u.`email` LIKE '%$search%')",[':cat_id'=>$cat_id])->queryOne();
        }
        else
        {
