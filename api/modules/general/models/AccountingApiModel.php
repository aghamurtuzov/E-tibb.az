...
class AccountingApiModel extends Model
{
    ...
    {
        return Yii::$app->db->createCommand("SELECT count(`id`) as `count` FROM `site_transaction_details` WHERE status=:status_",['status_'=>1])->queryOne();
    }

    public function AllPayments($limits)
    {
        return Yii::$app->db->createCommand("SELECT * FROM `site_transaction_details` WHERE status=:status_ ORDER BY id DESC LIMIT {$limits[0]},{$limits[1]}",['status_'=>1])->queryAll();
    }
