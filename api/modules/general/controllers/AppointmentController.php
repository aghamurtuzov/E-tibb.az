...
//    }
    ...
    /**
     * https://e-tibb.az/api/general/appointment/work-time-block
     * date=2019-11-04
     * time=12:00-12:30
     * doctor_id=123
     */
    public function actionWorkTimeBlock()
    {
        ...
        // Check Doctor exist
        $doctor = DoctorsApiModel::doctorFind($doctorID);
        if(!empty($doctor)) {
            $add = new SiteAppoint();
            $add->date = $date;
            $add->time = $time;
            $add->doctor_id = $doctorID;
            $add->user_id = $doctor['user_id'];
            $add->status = 1;
            if($add->save())
            {
                return $this->response(200,"Seçilmiş tarix bloklandı");
            }
            return $this->response(200,"Əməliyyatın icra olunması zamanı xəta baş verdi", $add->errors);
        } else {
            return $this->response(404, "Həkim tapılmadı");
        }
    ...
    /**
     * https://e-tibb.az/api/general/appointment/work-time-active
     * date=2019-11-04
     * time=12:00-12:30
     * doctor_id=123
     */
    ...
    public function actionWorkDay()
    {
        ...
        if(isset($l))
        {
            foreach($l['list'] as $key => $val)
            {
                if(isset($app[$val]))
                {
                    if($app[$val]['user_id'] != $doctor['user_id'])
                    {
