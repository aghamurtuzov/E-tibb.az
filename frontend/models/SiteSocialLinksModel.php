...
class SiteSocialLinksModel extends Model
{
    ...
    {
        $db = Yii::$app->db;
        $result = $db->createCommand("SELECT id,link,`link_type` FROM `".self::$tableName."` WHERE `connect_id`=:connect_id and `type`=:type",[":connect_id" => $connect_id,":type" => $type])->queryAll();
        return $result;
    }
