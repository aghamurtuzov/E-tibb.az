...
use api\modules\general\models\AuthItemChild;
use Yii;
...

{
    ...
    public static function NewsCategories($type)
    {
        $result = SiteMenus::find()->where(['type'=>$type,'hidden'=>1])->all();
        if(empty($result))
        {
            return null;
        }
    ...
    {
        return $this->hasMany(SiteMenus::className(), ['parent' => 'id'])->andWhere(['status' => 1]);
    }
